...
\caption{\textcolor{keywordcolor}{\textsc{FindMinimumCostPath}}}
\begin{algorithmic}[1]
\State \algorithmickeyword{Inputs:} $s$ (source node), $t$ (sink node), \textit{scalingFactor}, \textit{unitCost}, \textit{residualGraph}
\State \algorithmickeyword{Output:} \textit{minCostPath} (the minimum cost path from $s$ to $t$)
...
\caption{\textcolor{keywordcolor}{\textsc{ComputeMinCostPathsFromSource}}}
\begin{algorithmic}[1]
\State \algorithmickeyword{Inputs:} $s$ (source node), \textit{scalingFactor}, \textit{unitCost}, \textit{residualGraph}, \textit{parent}
\State \algorithmickeyword{Output:} Updates \textit{minCost} and \textit{parent} arrays to store shortest paths
...
%
%\begin{itemize}
%	\item \textbf{\textit{Shanmukha Venkata Naga Sai Tummala:}} Writing the code for GENERATE-SINK-SOURCE-GRAPH, SuccessiveShortestPathsSC (Algorithm 3), Simulations 1 - code for finding out graph characteristics (except Edmonds-Karp), Working on Simulations 2
%	\item \textbf{\textit{Mohamed Mohamed:}} Writing the code for Algorithm 4 (Primal Dual) and it's supporting methods and classes , Edmonds Karp - for Simulations, Test , Simulations 2, Working on Simulations 2
%	\item \textbf{\textit{Parsa Kamalipour:}} Writing the code for Algorithm 1 (Successive Shortest Paths), writing the report in LaTeX, Creating the tables, gathering Info and citing previous works, Working on Simulations 2
%	\item \textbf{\textit{Naveen Rayapudi:}} Writing the code for Algorithm 2 (Capacity Scaling), Working on Simulations 2
%\end{itemize}
%
%\textbf{NOTE:} We did Simulations 2 as a group. We have worked together in finding the patterns that we found in the outputs, recording them, creating inputs for Simulations 2, analyzing the results and noting down the observations
